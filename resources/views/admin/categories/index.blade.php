...
@section('content')
    <div class="flex items-center justify-between gap-4 mb-4">
        ...
        <div class="flex items-center gap-3">
            <form id="categories-filter-form" method="GET" action="{{ route('admin.categories.index') }}" class="flex items-center gap-2">
                <input type="search" name="q" value="{{ request('q') }}" placeholder="Pesquisar categorias" class="block p-2 border rounded-md text-sm" />
                <button type="submit" class="px-3 py-2 bg-blue-600 text-white rounded-md text-sm">Pesquisar</button>
                <a href="{{ route('admin.categories.index') }}" class="px-3 py-2 bg-gray-100 rounded-md text-sm clear-filters">Limpar</a>
            </form>
            <a href="{{ route('admin.categories.create') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium text-white bg-blue-600 hover:bg-blue-700">Nova Categoria</a>
        </div>
    </div>
    ...
    <div class="mt-4">
        @if($categories->total() > 0)
            <div class="text-sm text-gray-600 mb-2">Mostrando {{ $categories->firstItem() }} a {{ $categories->lastItem() }} de {{ $categories->total() }} resultados</div>
        @else
